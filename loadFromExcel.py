...
excel_file_name = "Malia.xlsx"
character_name = "Malia"
...
# Values from BinbinSubclass sheet, filled when that sheet is processed (for Setup.py)
setup_character_name = None
setup_subclass_id = None
...
def convert_xls_sheet_to_json(xls:pd.ExcelFile, sheet_name:str, output_directory):
    ...
    global character_name, setup_character_name, setup_subclass_id, setup_description
    ...
    if sheet_name == "BinbinSubclass":
        ...
        setup_character_name = character_name
        setup_subclass_id = str(df["ID"][0]).strip() if not pd.isna(df["ID"][0]) else "Snake"
        if "CharacterDescriptionStrength" in df.columns:
            desc_val = df["CharacterDescriptionStrength"][0]
            setup_description = "" if pd.isna(desc_val) else str(desc_val).strip()
        else:
            setup_description = ""
...
if __name__=="__main__":
    ...
    # Run Setup.py to replace placeholders with values from BinbinSubclass
    if setup_character_name is not None and setup_subclass_id is not None:
        ...
        if os.path.exists(setup_path):
            ...
            subprocess.run(
                [
                    sys.executable,
                    setup_path,
                    setup_character_name,
                    setup_subclass_id,
                    setup_description if setup_description is not None else "",
                ],
                cwd=script_dir,
                check=False,
            )
        else:
            print("Setup.py not found, skipping placeholder replacement.")
    else:
        print("BinbinSubclass data not available, skipping Setup.py.")
